    ...
    <div className="min-h-screen bg-[#0a0b0e] text-white p-4 md:p-6 font-black uppercase italic">
      ...
      <div className="relative z-10 max-w-[1850px] mx-auto">
        ...
        <div className="flex justify-between items-center mb-6 bg-[#111318]/80 backdrop-blur-md p-5 rounded-3xl border border-gray-800 shadow-2xl">
          <div>
            <h1 className="text-xl text-orange-500 tracking-tighter italic font-black">AI KOMBİNASYON MERKEZİ</h1>
            <p className="text-[9px] text-gray-500 mt-0.5 italic font-black">Saha 360 // AKILLI ÖĞRENME SİSTEMİ</p>
          </div>
        ...
        {/* İşlem Paneli */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mb-8 font-black uppercase italic">
          <div className="lg:col-span-7 bg-[#1a1c23] border border-gray-800 p-6 rounded-[2rem] shadow-2xl">
            <h3 className="text-orange-500 text-[9px] mb-3 tracking-widest font-black uppercase italic">✍️ YENİ KOMBİNASYON OLUŞTUR</h3>
            <textarea 
              value={manualKeywords} 
              onChange={(e) => setManualKeywords(e.target.value)}
              placeholder="KELİMELERİ VİRGÜL İLE AYIRIN (Örn: klima, fan, soğutma)..." 
              className="w-full h-20 bg-black border border-gray-700 p-3 rounded-xl text-[10px] outline-none focus:border-orange-500 text-white mb-3 italic font-black uppercase"
            />
            <div className="flex gap-2">
              <select value={manualTeam} onChange={(e) => setManualTeam(e.target.value)} className="flex-1 bg-black/50 border border-gray-600 p-2.5 rounded-xl text-[10px] text-orange-400 outline-none font-black italic">
                {calismaGruplari.map((ekip, i) => <option key={i} value={ekip.grup_adi} className="bg-black">{ekip.grup_adi}</option>)}
              </select>
              <button onClick={grupEkle} className="bg-orange-600 px-8 rounded-xl text-[10px] hover:bg-orange-500 font-black uppercase italic transition-all">MÜHÜRLE</button>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 bg-[#1a1c23] border border-blue-500/20 p-6 rounded-[2rem] shadow-2xl flex flex-col justify-center items-center text-center">
            <h3 className="text-blue-400 text-[9px] tracking-widest mb-4 font-black uppercase italic">📊 TOPLU VERİ YÜKLEME (EXCEL)</h3>
            <label className="w-full cursor-pointer bg-blue-600/5 border-2 border-dashed border-blue-500/20 p-5 rounded-2xl hover:bg-blue-600/10 transition-all group">
              ...
              <span className="text-[9px] text-blue-400 font-black uppercase italic">{uploading ? 'YÜKLENİYOR...' : 'DOSYAYI BURAYA BIRAK'}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 space-y-4 font-black uppercase italic">
            <div className="flex items-center gap-3 border-l-4 border-orange-600 pl-3">
              <h2 className="text-orange-500 text-xs font-black italic uppercase">🔥 SAHADAN GELEN ÖNERİLER (OTOMATİK)</h2>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 font-black italic uppercase">
              {hamKelimeler.map((item) => (
                <div key={item.id} className="bg-[#111318] p-3 rounded-xl border border-orange-500/10 hover:border-orange-500/50 transition-all font-black italic uppercase">
                  <div className="flex flex-wrap gap-1">
                    {item.kelime_grubu.map((k: string, i: number) => (
                      <span key={i} onClick={() => setManualKeywords(prev => prev ? `${prev}, ${k}` : k)} className="bg-orange-600 text-white px-2 py-1.5 rounded-md text-[10px] font-black cursor-pointer w-full text-center hover:scale-105 transition-transform uppercase italic">
                        {k}
                      </span>
                    ))}
                  </div>
                  <div className="flex justify-between items-center mt-2 pt-2 border-t border-gray-800 font-black italic uppercase">
                    <span className="text-[8px] text-gray-500 font-black italic">YENİ ÖNERİ</span>
                    <button onClick={() => sil(item.id, true)} className="text-[8px] text-red-500 font-black italic uppercase hover:text-red-400">SİL</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* SAĞ: ONAYLANANLAR */}
          <div className="lg:col-span-5 space-y-4 font-black italic uppercase">
            <div className="flex items-center gap-3 border-l-4 border-blue-600 pl-3 font-black italic uppercase">
              <h2 className="text-blue-500 text-xs font-black italic uppercase">✅ AKTİF AI KURALLARI</h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 font-black italic uppercase">
              {onayliKombinasyonlar.map((item) => (
                <div key={item.id} className="bg-[#111318] p-3 rounded-xl border border-gray-800 hover:border-blue-500/40 transition-all font-black italic uppercase">
                  <div className="flex flex-wrap gap-1 mb-2">
                    {item.kelime_grubu.map((k: string, i: number) => (
                      <span key={i} className="bg-blue-500/10 text-blue-400 px-2 py-1 rounded-md text-[9px] border border-blue-500/20 font-black uppercase italic">
                        #{k}
                      </span>
                    ))}
                  </div>
                  <div className="flex justify-between items-center border-t border-gray-800/50 pt-2 font-black italic uppercase">
                    <span className="text-[9px] text-blue-400 uppercase font-black italic">{item.onerilen_ekip}</span>
                    <button onClick={() => sil(item.id, false)} className="text-[9px] text-red-500 font-black italic uppercase hover:text-red-400">KALDIR</button>
                  </div>
                </div>
              ))}
            </div>
