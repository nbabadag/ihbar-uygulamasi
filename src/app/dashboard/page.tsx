...
export default function DashboardPage() {
    ...
    if (ihbarData) {
      ...
      if (role.trim().toUpperCase() === 'SAHA PERSONELI') {
        filtered = ihbarData.filter(i => {
          ...
          // 2. Mesai dışıysa ve Vardiya Modu etiketi varsa otomatik havuzda gör
          const vardiyaModundaGor = !isMesaiSaatleri && i.oncelik_durumu === 'VARDİYA_MODU' && i.durum === 'Beklemede';

          return kendisineAtanmis || vardiyaModundaGor;
        });
      } else {
        // Admin ve Yetkililer her şeyi görür
        filtered = ihbarData;
      }
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setNow(new Date());
      // Her dakika başı saat kontrolü için veriyi tazele
    ...
    <div className="min-h-screen flex flex-col md:flex-row text-white font-sans relative overflow-hidden bg-[#0a0b0e]">
      ...
      <div className="hidden md:flex w-64 bg-[#0a0b0e]/95 backdrop-blur-2xl text-white shadow-2xl flex-col fixed h-full z-50 border-r border-gray-800/30">
        ...
        <div className="px-4 flex flex-col flex-1">
          <nav className="space-y-2.5 flex-1 font-black text-[11px] overflow-y-auto custom-scrollbar uppercase italic tracking-tighter">
            <button onClick={() => router.push('/dashboard/saha-haritasi')} className="w-full text-left p-4 bg-orange-600 hover:bg-orange-700 rounded-2xl flex items-center gap-3 transition-all shadow-xl mb-6 active:scale-95 shadow-orange-900/20 text-white">
            ...
            {canCreateJob && <div onClick={() => router.push('/dashboard/yeni-ihbar')} className="p-3 hover:bg-gray-800/50 rounded-xl cursor-pointer transition-all opacity-80 hover:opacity-100 text-white">📢 İhbar Kayıt</div>}
            {canManageUsers && <div onClick={() => router.push('/dashboard/personel-yonetimi')} className="p-3 hover:bg-gray-800/50 rounded-xl cursor-pointer transition-all opacity-80 hover:opacity-100 text-white">👤 Personel Yönetimi</div>}
            ...
            {canManageGroups && <div onClick={() => router.push('/dashboard/calisma-gruplari')} className="p-3 hover:bg-gray-800/50 rounded-xl cursor-pointer transition-all opacity-80 hover:opacity-100 text-white">👥 Çalışma Grupları</div>}
            {canSeeTV && <div onClick={() => router.push('/dashboard/izleme-ekrani')} className="p-3 bg-red-600/10 text-red-500 border border-red-900/30 rounded-xl cursor-pointer animate-pulse text-center mt-4 text-[10px] font-black">📺 İzleme Ekranı</div>}
            {canSeeReports && <div onClick={() => router.push('/dashboard/raporlar')} className="p-3 hover:bg-gray-800/50 rounded-xl cursor-pointer transition-all opacity-80 hover:opacity-100 text-white">📊 Raporlama</div>}
          </nav>
      ...
      <div className="flex-1 p-4 md:p-8 ml-0 md:ml-64 font-bold flex flex-col gap-6 relative z-10 font-black">
        <div className="md:hidden flex justify-between items-center bg-[#111318] p-4 rounded-2xl text-white shadow-xl border border-gray-800 font-black">
          ...
          <button onClick={() => setIsBildirimAcik(true)} className="relative p-2"><span className="text-xl">🔔</span>
