...
export default function Raporlar() {
  ...
  const fetchStats = async () => {
    ...
    if (ihbarlar) {
      setRawData(ihbarlar);
      // Başlangıçta "Diğer" hariç tüm benzersiz nesneleri seçili getir
  ...
  // 📥 EXCEL (CSV) İNDİRME
  const excelIndir = () => {
    if (filteredData.length === 0) return alert("İndirilecek veri yok.");
    ...
    link.href = url;
    link.download = `Saha360_Ozel_Rapor.csv`;
    link.click();
  }
      ...
      <div className="max-w-[1600px] mx-auto space-y-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center gap-6 border-b-2 border-orange-500 pb-8">
          ...
          <div className="flex flex-wrap items-center gap-4">
            {/* 🛠️ NESNE FİLTRELEME MENÜSÜ */}
            <div className="relative">
              <button 
                onClick={() => setShowFilter(!showFilter)}
                className="bg-[#111318] border border-gray-800 px-6 py-4 rounded-2xl text-[10px] flex items-center gap-3 hover:border-orange-500 transition-all"
              >
                ⚙️ NESNELERİ FİLTRELE ({selectedNesneler.length})
              </button>
              ...
              {showFilter && (
                <div className="absolute top-full left-0 mt-2 w-80 max-h-96 bg-[#111318] border border-gray-800 rounded-3xl p-6 z-[100] shadow-2xl overflow-y-auto">
                  <div className="flex justify-between mb-4 border-b border-gray-800 pb-2">
                    <button onClick={() => setSelectedNesneler(tumNesneListesi)} className="text-[8px] text-blue-500">HEPSİNİ SEÇ</button>
                    <button onClick={() => setSelectedNesneler([])} className="text-[8px] text-red-500">TEMİZLE</button>
                  </div>
                  {tumNesneListesi.map(nesne => (
                    <label key={nesne} className="flex items-center gap-3 mb-3 cursor-pointer group">
                      <input 
                        type="checkbox" 
                        checked={selectedNesneler.includes(nesne)} 
                        onChange={(e) => {
                          if(e.target.checked) setSelectedNesneler([...selectedNesneler, nesne]);
                          else setSelectedNesneler(selectedNesneler.filter(n => n !== nesne));
                        }}
                        className="accent-orange-500"
                      />
                      <span className="text-[9px] group-hover:text-orange-500 transition-all">{nesne}</span>
                    </label>
                  ))}
                </div>
              )}
            ...
            <div className="flex items-center gap-2 bg-[#111318] p-4 rounded-3xl border border-gray-800">
              <input type="date" className="bg-transparent text-[10px] text-white outline-none" value={startDate} onChange={e => setStartDate(e.target.value)} />
              <input type="date" className="bg-transparent text-[10px] text-white outline-none" value={endDate} onChange={e => setEndDate(e.target.value)} />
              <button onClick={excelIndir} className="bg-green-600 px-6 py-2 rounded-2xl text-[10px] ml-2">📊 EXCEL</button>
              <button onClick={() => router.push('/dashboard')} className="bg-orange-600 px-6 py-2 rounded-2xl text-[10px]">GERİ</button>
            </div>
          </div>
        </div>

        {/* KPI KARTLARI (DİNAMİK) */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-[#111318] p-8 rounded-[2rem] border-l-4 border-orange-600">
            <p className="text-[10px] text-gray-500 mb-2">SEÇİLİ EKİPMAN İŞİ</p>
            <h2 className="text-6xl text-orange-500 tracking-tighter font-black">{stats.total}</h2>
          </div>
          <div className="bg-[#111318] p-8 rounded-[2rem] border-l-4 border-blue-500">
            <p className="text-[10px] text-gray-500 mb-2">ORT. TAMİR (MTTR)</p>
            <h2 className="text-6xl text-blue-500 tracking-tighter font-black">{stats.avgTime} DK</h2>
          </div>
          <div className="bg-[#111318] p-8 rounded-[2rem] border-l-4 border-green-500">
            ...
            <h2 className="text-6xl text-green-500 tracking-tighter font-black">%{stats.efficiency}</h2>
          </div>
        ...
        {/* GRAFİK BÖLÜMÜ */}
        <div className="bg-[#111318] p-10 rounded-[3rem] border border-gray-800 h-[500px] shadow-2xl relative">
          <h3 className="text-xs border-l-4 border-blue-500 pl-4 tracking-[0.2em] mb-10">ÖZEL FİLTRELİ DARBOĞAZ ANALİZİ</h3>
              ...
              <Tooltip 
                content={({ active, payload }: any) => {
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-black border border-orange-500/50 p-4 rounded-xl shadow-2xl font-black italic uppercase text-[10px]">
                        <p className="text-orange-500 border-b border-gray-800 pb-1 mb-2">{item.name}</p>
                        <p>ARIZA: <span className="text-blue-400">{item.x}</span></p>
                        <p>SÜRE: <span className="text-red-400">{item.y} DK</span></p>
                      </div>
                    );
                  }
                  return null;
        ...
        {/* KRİTİK LİSTE */}
        <div className="bg-[#111318] p-10 rounded-[4rem] border border-gray-800">
          <h3 className="text-xs mb-8 border-l-4 border-red-600 pl-4 tracking-[0.2em]">SEÇİLİ EKİPMANLARDA EN KRİTİK 10 VARLIK</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            {scatterData.slice(0, 10).map((e, idx) => (
              <div key={idx} className="bg-black/50 p-8 rounded-[3rem] border border-gray-900 relative hover:border-orange-500/50 transition-all">
