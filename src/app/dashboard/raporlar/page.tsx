...
export default function Raporlar() {
  ...
  const fetchStats = async () => {
    ...
    if (ihbarlar) setRawData(ihbarlar);
    setLoading(false);
  }

  // 🛡️ KRİTİK FİLTRELEME MANTIĞI (Hem "Diğer" Hariç, Hem Tarih Aralığına Göre)
  const filteredData = useMemo(() => {
    return rawData.filter(i => {
      // 1. "Diğer" nesnesini her dilde ve her harf boyutunda dışla
      const nesneAdi = (i.secilen_nesne_adi || '').trim().toUpperCase();
      if (nesneAdi === 'DİĞER' || nesneAdi === 'DIGER' || nesneAdi === '') return false;

      // 2. Tarih filtresini uygula (eğer seçiliyse)
  ...
  // 🏁 KPI HESAPLAMALARI (Sadece filtrelenmiş veriden)
  const stats = useMemo(() => {
  ...
  const excelIndir = () => {
    ...
    link.href = url;
    link.download = `Saha360_Rapor_${startDate || 'Tumu'}.csv`;
    link.click();
  }

  // 📊 GRAFİK VERİLERİ (Diğer Hariç)
  const workshopData = [
    { name: 'ELEKTRİK (201)', value: filteredData.filter(d => d.is_istasyonu?.includes('201')).length, color: '#3b82f6' },
    { name: 'MEKANİK (202)', value: filteredData.filter(d => d.is_istasyonu?.includes('202')).length, color: '#ea580c' },
    { name: 'BİNA (204)', value: filteredData.filter(d => d.is_istasyonu?.includes('204')).length, color: '#10b981' },
  ];
      ...
      <div className="max-w-[1600px] mx-auto space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-[#111318] p-8 rounded-[2rem] border-l-4 border-orange-600">
            <p className="text-[10px] text-gray-500 mb-2 font-black tracking-widest">TOPLAM EKİPMAN İŞİ</p>
          ...
          <div className="bg-[#111318] p-8 rounded-[2rem] border-l-4 border-blue-500">
            <p className="text-[10px] text-gray-500 mb-2 font-black tracking-widest">GERÇEK MTTR</p>
            <h2 className="text-6xl text-blue-500 tracking-tighter">{stats.avgTime} DK</h2>
          </div>
          ...
          <div className="bg-[#111318] p-8 rounded-[2rem] border-l-4 border-red-500">
            <p className="text-[10px] text-gray-500 mb-2 font-black">EN KRİTİK NESNE</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 bg-[#111318] p-10 rounded-[3rem] border border-gray-800">
            <h3 className="text-xs mb-10 border-l-4 border-orange-500 pl-4 tracking-widest">İŞ İSTASYONU YÜKÜ</h3>
          ...
          <div className="lg:col-span-2 bg-[#111318] p-10 rounded-[3rem] border border-gray-800 h-[500px]">
            <h3 className="text-xs mb-10 border-l-4 border-blue-500 pl-4 tracking-widest">DARBOĞAZ ANALİZİ (ÖLÇEKLİ)</h3>
            <ResponsiveContainer width="100%" height="80%">
              <ScatterChart margin={{ top: 20, right: 30, bottom: 20, left: 10 }}>
                <CartesianGrid stroke="#222" vertical={false} strokeDasharray="3 3" />
                {/* 🚀 X EKSENİ: 100'e kadar geniş, sonrası logaritmik sıkıştırma */}
        ...
        {/* 🏆 EN KRİTİK 10 EKİPMAN */}
        <div className="bg-[#111318] p-10 rounded-[4rem] border border-gray-800 shadow-2xl">
          <h3 className="text-xs mb-8 border-l-4 border-red-600 pl-4 tracking-widest">OPERASYONU DURDURAN 10 VARLIK</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            {top10Equipment.map((e, idx) => (
              <div key={idx} className="bg-black/50 p-8 rounded-[3rem] border border-gray-900 relative group hover:border-orange-500/50 transition-all">
                ...
                <p className="text-[10px] text-gray-500 mb-6 h-8 overflow-hidden font-black leading-tight uppercase group-hover:text-white">{e.name}</p>
                <div className="flex justify-between items-end border-t border-gray-800 pt-4">
                  <div>
                    <span className="text-[8px] text-gray-700 font-black tracking-widest">ARIZA</span>
                    <p className="text-3xl font-black text-white">{e.x}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-[8px] text-gray-400 font-black">ORT. SÜRE</span>
